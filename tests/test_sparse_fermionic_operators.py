"""Test exact diagonalization of fermionic sparse matrices."""
...
class SparseManyBodyOperatorsTestCase(unittest.TestCase):
    ...
    def test_values_in_symmetric_operator_matrix(self):
        ...
        for j in range(5):
            for i in range(j):
                J[j, i] = J[i, j]
                D[j, i] = D[i, j]
        v, r, c, ns = fer_sp_sym_pc_op(5, 3, J, D)
        H = csr_matrix((v, (r, c)), shape=(ns, ns)).toarray()
    ...
    def test_values_in_npc_operator(self):
        ...
        r = np.arange(1, 5)
        l = np.arange(5, 9)
        v, r, c, ns = fer_sp_npc_op(4, J, D, r, l)
        H = csr_matrix((v, (r, c)), shape=(ns, ns)).toarray()
    ...
    def test_values_in_number_conserving_correlation_matrix(self):
        ...
        v, r, c, ns = fer_sp_pc_correlator(5, 3, 0, 2)
        ...
        v, r, c, ns = fer_sp_pc_correlator(5, 3, 1, 3)
        C = csr_matrix((v, (r, c)), shape=(ns, ns)).toarray()
        ...
        v, r, c, ns = fer_sp_pc_correlator(5, 3, 4, 3)
        C = csr_matrix((v, (r, c)), shape=(ns, ns)).toarray()
    ...
    def test_values_in_number_nonconserving_correlation_matrix(self):
        ...
        v, r, c, ns = fer_sp_npc_correlator(4, 0, 2)
        C = csr_matrix((v, (r, c)), shape=(ns, ns)).toarray()
        ...
        v, r, c, ns = fer_sp_npc_correlator(4, 1, 2)
        C = csr_matrix((v, (r, c)), shape=(ns, ns)).toarray()
        ...
        v, r, c, ns = fer_sp_npc_correlator(4, 3, 2)
        C = csr_matrix((v, (r, c)), shape=(ns, ns)).toarray()
